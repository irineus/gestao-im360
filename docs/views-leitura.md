...
|---|---|---|---|---|
| 1 ✅ | Criar `fn_hoje()` e trocar **todo** `current_date` de view, função e rotina por ela — **feito no card 3.4** (`20260901163000_acesso_rls.sql`), e o C6 do `011` reprova `current_date` novo | 2.1 §3, 2.2, 2.5 | 3.4 | **bloqueante** para a correção dos números à noite (§3.3) |
...
| 6 ✅ | `aluno_status_ix` de `(unidade_id, status)` para `(unidade_id, metodo_id, status)`, que é como o dashboard agrupa — **feito no card 4.2** (`20260902210000_alunos_status.sql`) | 2.1 §7 | 4.2 | baixa — desempenho |
...
| `v_projecao_acuracia` | `estoque.ler`, `alunos.ler` — sem `estoque.ler` vem **vazia** (nenhum mês medido); sem `alunos.ler` o previsto aparece inteiro como sobra (card 11.1,5) |
...
| `v_projecao_acuracia` | **11.1,5** ✅ — `20260924190000_projecao_acuracia.sql`, antecipada do 11.2 (que fica só com a recalibração); duas divergências de correção em `projecao-demanda.md` §9.3. Teste em `098_projecao_acuracia`, 11 asserções | 11 |
