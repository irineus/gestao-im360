...
| 28 | **O §7.3 fixa o cabeçalho da Projeção em "calculada em {calculado_em}" e não diz o que fazer sem carimbo** | ⚠️ **Corrigido em 06/09/2026 (card 9.2,5): na Projeção são DOIS estados, não três.** O carimbo sai da **mesma leitura** que a grade (`calculadoEmDe(celulas)`), então "não foi possível ler quando foi calculada" nunca acontece sozinho ali — quando a grade falha, a tabela já mostra o erro com "Tentar de novo", e o cabeçalho desenhava uma **segunda** frase de erro para a mesma falha. Em Compras o terceiro estado continua existindo, porque lá o carimbo é leitura à parte. Enunciado original: São **três** estados, os mesmos que o card 8.2 fixou para Compras e pela mesma razão: calculada, **nunca calculada** ("A projeção ainda não foi calculada. Ela é atualizada pela rotina da madrugada.") e carimbo ilegível. Enquanto carrega, **nada** é desenhado — piscar "ainda não foi calculada" por meio segundo diz uma coisa falsa —, e o texto vai num `Flexible`, que é a correção 19 aplicada antes de doer |
...
### Correções e achados do card 9.2,5 (06/09/2026) — revisão das telas 08 e 09

As cinco telas dos cards 8.2, 8.5, 8.6, 8.7 e 9.1 foram revisadas contra este documento (§3,
§5.2–§5.8, §7, §8, §11). Sem migração. O que este documento passou a dizer com mais precisão:

| # | Achado | Como ficou |
|---|---|---|
| 42 | **O §5.6 não dizia o que a tabela faz na RECARGA** (toda escrita seguida de `versaoX++`), e o `TabelaIm360` fazia `when(loading:)` sem `skipLoadingOnReload` — o padrão do Riverpod é mostrar `loading` mesmo com o valor anterior em mãos | **A recarga NÃO volta ao esqueleto.** Medido em 390 px, na jornada nº 2 do monitor: tocar a caixa Financeiro de um aluno fazia a fila inteira sumir (`EstadoCarregando = 1`, cartão = 0) pelo tempo de uma ida ao banco, a rolagem voltava ao topo e a próxima caixa que o monitor ia tocar mudava de lugar. Agora: primeira carga com esqueleto; recarga com as linhas anteriores na tela e um `LinearProgressIndicator` de 2 px no topo; erro continua vindo antes de tudo (`hasError`, correção 6). Vale para as **nove** telas atrás do componente e para os dois carimbos da projeção (Compras e Projeção), que **somem e voltam** a cada recarga da aba e também passaram a ficar |
| 43 | ⚠️ **ACHADO DE FRAMEWORK — o `whenData` do Riverpod 3.4.2 DESCARTA o valor anterior na recarga.** Toda tela filtrava a lista antes de entregá-la à tabela com `leitura.whenData((l) => filtrar(l, filtro))`, e o ramo `loading` do `whenData` devolve um `AsyncLoading` **sem** valor (`async_value.dart`). A correção 42 no componente **não chegava a nenhuma tela que filtra** — medido no `tela_certificados_test`: a fila continuava sumindo | Nasceu `derivar` em `lib/util/async_valor.dart`: mapeia em `data`, repassa `error`, e em `loading` **com** valor devolve um `loading` com o valor **mapeado**. As doze derivações de `lib/telas/` passaram a usá-lo. ⚠️ O único jeito de construir "carregando com valor" fora do pacote é o `copyWithPrevious`, que o Riverpod marca como interno — o `ignore` fica **num lugar só**, e `test/async_valor_test.dart` mede o comportamento num provider de verdade: se uma versão do Riverpod mudar isto, é ele que avisa. Nenhuma tela usa `whenData` para linhas de tabela desde então |
| 44 | **O §5.4 diz que "em execução, o primário trava reenvio", e a caixa Financeiro da LISTA da tela 9 não travava** — dois toques com 50 ms de intervalo mandavam **duas** escritas iguais (o painel já tinha a trava) | A tela guarda o conjunto de alunos com escrita em curso; enquanto o aluno está nele a caixa fica desabilitada **com o motivo** ("Gravando…" — é estado, não permissão, §5.7). E o erro dessa escrita deixou de ir para snackbar de 4 s: vira `AvisoTonal(erro: true)` acima da tabela, **com o nome do aluno**, como o painel já fazia. A confirmação também diz de quem foi ("Financeiro OK marcado para {nome}.") — com dez alunos na fila, "Checklist atualizado." não dizia qual |
| 45 | **`AvisoTonal` não tinha ação**, e uma leitura secundária que falha (o catálogo de métodos da Projeção e de Certificados) não tinha como oferecer "Tentar de novo" sem derrubar a tela em `EstadoErro` | Ganhou `rotuloAcao`/`aoAgir`: um `TextButton` à direita, na cor do banner. É o que faz o item B3 caber: com `metodosProvider.hasError`, a Projeção diz "não lido" na coluna Método e o aviso acima da tabela oferece reler; Certificados só perde o filtro por método, e o aviso diz isso |
| 46 | **Alvos abaixo do §8.4 em três lugares**: os números do cartão de alunos do Dashboard (24 px — e no celular são os únicos atalhos da tela), a célula do mês da Projeção (~28 px numa linha de 44) e a linha do aluno no drill-down **sem `Semantics(button:)`** (§8.5) | `_Alvo` do Dashboard com `minHeight` de 40 no desktop e 44 no celular; a célula do mês ocupa a altura da linha; a linha do aluno anuncia "{aluno}, abrir a ficha". Os três medidos por `getSize`/`bySemanticsLabel` nos testes |
| 47 | **`Opacity(0.6)` no passo inativo da Importação reprovava AA** (§8.1): esmaecia **o texto junto**, e `onSurfaceVariant` (5,06:1) caía para **2,36:1** — nos textos "Valide o arquivo no passo 2…" e "Escolha um arquivo no passo 1…", que são os que a pessoa precisa ler para saber o que falta | Esmaece só a **moldura e o número** (borda mais clara, círculo em `surfaceContainerHighest`) e o texto fica em `onSurfaceVariant` **pleno**. O teste assere que o `Text` do passo inativo não está dentro de um `Opacity < 0.85` |
| 48 | **`AsyncValue` que decide texto precisa dos três estados** (correção do 5.11 e do 8.1,5), e a Importação lia `loteImportacaoProvider(id).value` — `loading` e `error` viravam "ainda não há lote", e o passo 4 dizia "Valide o arquivo no passo 2" a quem acabou de validar (medido: 1 texto falso, 0 `EstadoErro`) | Observa o `AsyncValue` inteiro: `hasError` → `EstadoErro` compacto com "Tentar de novo" nos passos 3 e 4; `isLoading` sem valor → esqueleto de uma linha; o texto "Valide…" só quando não há lote. O motivo do "Baixar relatório" também passou a ser por estado ("O relatório ainda está carregando."), e em erro o botão fica ausente — a tabela já mostra o `EstadoErro` |
| 49 | **Três tabelas de 360 px fixos dentro da página rolável da tela 13** — no celular a rolagem da página e a da tabela disputam o dedo | **Registrado, não alterado.** A tela 13 é de desktop (`wireframes.md` §17, divergência 69: no celular não há seletor de arquivo), e uma lista de ocorrências que crescesse com o conteúdo empurraria o passo 4 e o histórico para fora de vista num arquivo de mil linhas. A altura virou **uma constante nomeada** (`_alturaTabela`), e não `360` três vezes |
...
| 54 | **Três extensões de componente para a jornada do monitor** (card 9.2,64) | `CartaoIm360.informacao` + `iconeInformacao`: uma linha em cor de texto, com ícone, entre o subtítulo e o apoio (o "próximo livro"); `TabelaIm360.buscaMobile` / `BarraFiltrosIm360.buscaMobile`: um campo que no celular fica à vista acima de "Filtrar", fora da folha; `MenuItemEntregue`: o menu "⋮" do item entregue da Trilha, com o item indisponível desabilitado e o motivo no lugar do rótulo. ⚠️ **Armadilha medida:** o `late final` preguiçoso do controlador de busca dos filtros passou a ser criado no `dispose` quando a busca saiu da folha — `ref.read` em widget desmontado, StateError; foi para o `initState` |
| 55 | **`FileiraCartoes` — cartões de método dividindo a largura** (card 9.2,71) | Os cartões do Dashboard tinham largura fixa de 200 px, e em 1280 px três deles ocupavam menos da metade da linha. `FileiraCartoes` (widgets/card_dashboard.dart): no desktop/tablet, até 4 cartões dividem a linha com `Expanded`; acima disso, `Wrap` de largura fixa; no mobile empilham, como antes. Mesma decisão registrada no wireframes §17 (item 76) |
| 56 | **`Dim.alvoMobile` continua 44 px, e não os 48 dp do Material** (card 9.2,72, avaliação pedida pela revisão de 24/09/2026) | Avaliado e **mantido**. 44 px é o AAA do WCAG 2.5.5 e o mínimo do HIG; os botões do Material, na densidade padrão que o app usa no celular, já ganham 48 dp de ALVO pelo `materialTapTargetSize` (medido pelo `diretrizes_acessibilidade_test`), e os 44 valem para os alvos próprios (`InkWell` de cartão, célula e linha). Subir para 48 reabriria todos os leiautes medidos em 390 px sem ganho que um teste acuse. O teste de diretrizes confere 44 px no celular e os 40 px de `Dim.alturaBotao` no desktop (ponteiro). |
---
