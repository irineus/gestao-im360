...
| **(fora deste documento)** | `importacao`, `importacao_ocorrencia`, `importacao_referencia` | 9.1 ✅ — criadas em 06/09/2026, e **este DDL não as previa**: em 31/08/2026 a carga da planilha ainda seria script + `psql`, e a decisão de 02/09/2026 a transformou em operação de tempo de execução, disparada de uma tela. Operação de tempo de execução deixa rastro em tabela. Seguem as convenções (`unidade_id`, quatro colunas de auditoria, trigger de auditoria, RLS forçada), com duas ausências que são decisão: **nenhuma tem `delete`** — apagar o relatório apaga a explicação de como a escola entrou no sistema, e apagar o mapa de chave externa faz a importação SEGUINTE duplicar movimento de estoque, que é imutável — e **só `importacao` tem `update`**, porque só ela muda depois de nascer. As seis políticas usam o mesmo guarda, `fn_importacao_pode()`. Detalhe em `docs/importacao.md` |

Ordem de dependência entre migrações: 3.3 → 3.4 → 3.6 → 4.1 → 4.2 → 4.3 → 5.1 → 5.5 → 6.1 → 7.1 → 8.3 → 9.1.
