...
| RLS não é por coluna; trigger de colunas do checklist (2.4) | ✅ `081` §4 — monitor tenta `PATCH` em `pedagogico_ok` → recusa pelo trigger, não pela política, com a contraprova vermelha ao lado |
| "Quem e quando" de cada item vale para o `PATCH` (2.2 §8) | ✅ `081` §5 — o par é reescrito com o valor antigo quando o item não muda; sem o `else`, a autoria vira a de outra pessoa |
| Formar o aluno fecha `ALUNO_ULTIMO_LIVRO` e `SUGERIR_FORMADO` (2.2 §10.1) | ✅ `081` §8 — sem o trigger, as duas ficam abertas para sempre |
...
|---|---|---|
| `seed.sql` (pgTAP, schema `tests`, helpers, escola-fixture) | 3.3 (bootstrap) → **3.4.5** (helpers e camada `acesso`) → cresceu em 3.6, 4.1, 4.2, 4.3, 5.1, 6.1, 7.1 e **8.3** ✅ (camada `certificados`: o checklist de João Pedro com os itens em estados diferentes — pedagógico e financeiro marcados, formatura NÃO, certificado ENTREGUE). ⚠️ **Divergência registrada (8.3): a camada `certificados` é a ÚLTIMA declarável**, porque `certificado_checklist` é a última tabela do DDL do card 2.1 — os cards restantes são função, tela e importação. A instrução do `001` §7 («aplicada uma camada, declarar uma NOVA para o portão vigiar») deixou de ser aplicável, e a prova por construção passou a **desmarcar uma camada aplicada** dentro da transação em vez de criar a tabela de uma futura: mede o mesmo mecanismo, sem depender de haver uma próxima | 3+ |
...
| `080_projecao_demanda` (`v_ritmo_aluno` com os dois filtros e a janela, os quatro degraus da cascata com um aluno nomeado em cada, a ORDEM entre eles, a **disjunção** `k >= 2`, as datas de cada degrau, a extrapolação do cronograma Modular, e a rotina: contexto, janela, idempotência, foto mensal e a pendência de cronograma abrindo e fechando) | **8.1** ✅ — o §17 previa o nome `080_projecao`; o arquivo nasceu `080_projecao_demanda`, para casar com o nome da migração e do documento. ⚠️ **Quatro contraprovas vistas vermelhas:** sem `where k >= 2` a disjunção cai (e a previsão de Carla passa a começar em `hoje + 30`); sem o limite de um ritmo na âncora, Ana Paula salta de `hoje + 60` para `hoje + 30`; sem o piso e o teto, João Pedro vai de `90/2` para `60/3` (entrega em lote) e `117/3` (volta de parada), que são as duas falhas OPOSTAS do §4.2 na mesma medida; e com a política de `insert` por `tem_permissao` em vez de `fn_contexto_rotina()`, a direção grava projeção pelo PostgREST. ⚠️ **A fixture cresceu junto, e por necessidade estrutural, não por conveniência:** com o combo de Informática em três materiais, "duas entregas" e "dois itens pendentes" eram mutuamente exclusivos e o degrau RITMO_ALUNO era inalcançável; no Modular, o curso de um livro só deixava todo aluno em `k = 1`. Dois materiais novos e uma previsão de conclusão depois, os quatro degraus têm aluno — e vinte asserções de premissa mudaram de valor nos arquivos `023`, `050`, `051`, `052`, `071`, `090` e `095`. ⚠️ **E a suíte de concorrência reprovou no CI por causa da fixture, não do código:** `INTERATIVO 04` nasceu com quatro exemplares e virou o "próximo item pendente com estoque" de Ana Paula e Bruno, que são os dois alunos da corrida de `entrega_ultimo_exemplar.sh` — quem perdia passou a **reordenar** em vez de sair com `BLOQUEADA_SEM_ESTOQUE`, e o script reprovou com razão. Corrigido dando ao `04` **um** exemplar, entregue a João Pedro: a fixture passa a ter **três** saldos zero, e o terceiro existe para manter aquele desfecho. ⚠️ **Ganhou a §10 no card 8.2** ✅ (05/09/2026, 54 → **60** asserções): a parcela projetada chegando a `v_pedido_sugerido`. Mora aqui, e não no `095`, porque a asserção só existe **depois** de `rt_projecao_demanda()` ter rodado — e quem a roda é este arquivo; o `095` mede a mesma view com a projeção vazia, que é o outro caso, e passou a dizer isso em voz alta. **Três contraprovas vistas vermelhas:** revertendo só o `greatest` (o total deixa de fechar com as parcelas que a própria linha exibe), revertendo só a coluna (o mesmo teste cai pelo lado oposto, e mais dois com ele) e alargando a janela em 400 dias para os dois lados (a projeção de fora do horizonte entra na compra). ⚠️ **E o §10 precisou de contexto de unidade, o que é novo para uma leitura de view:** desde o 8.2 a view chama `fn_param_int('projecao_horizonte_dias')`, que é por unidade — sem contexto a leitura não devolve zero, **morre** com `PARAMETRO_AUSENTE`. Usa-se `tests.como_rotina`, que dá a unidade pela GUC sem trocar de papel; trocar para `authenticated` tornaria o schema `tests` inalcançável dali para a frente, que é a pedra da §8 deste mesmo arquivo | 8 |
| `081_certificado_checklist` (a camada `certificados` da fixture; o achado 3 do card 2.4 §7 medido contra o SEED; `fn_certificado_abrir` com idempotência, a data derivada e os dois erros; a **guarda de coluna atacada por `update` direto, perfil a perfil**; os pares quem/quando preenchidos pelo trigger e não falsificáveis por PATCH; os códigos das duas funções de marcação; `tg_certificado_sugere_formado` nos dois lados com o aluno continuando ATIVO; o fechamento das duas pendências na formatura; e `fn_certificado_reavaliar_estorno` nos dois desfechos — `MANTIDO_INCONSISTENTE` pelo caminho REAL, `fn_estornar_entrega`) | **8.3** ✅ — o §17 não previa arquivo para o 8.3; card de Função/regra tem obrigação própria no §13, e ela é cumprida aqui. Mora no bloco `08x`, ao lado do `080`. ⚠️ **Quatro contraprovas vistas vermelhas, e uma delas VERDE primeiro:** a guarda sem o grupo do pedagógico (o monitor reescreve `pedagogico_ok` por PATCH), o quem/quando sem o `else` (a autoria é trocada pela de outra pessoa), o trigger de formatura removido (as duas pendências ficam abertas) e `fn_certificado_reavaliar_estorno` como `invoker` — **esta passou VERDE na primeira tentativa**, porque a asserção rodava em `tests.como_rotina`, que mantém a sessão como `postgres` e portanto com BYPASSRLS. Reescrita na pele da direção (quem estorna de verdade), a sabotagem ficou vermelha do jeito certo: a linha sobrevive e a função ainda devolve `APAGADO`. É o mesmo achado do card 5.4 — contraprova verde é informação, não formalidade | 8 |
| `082_projecao_views_tela` (as duas views da tela 8 e a coluna `v_projecao_aluno.ritmo_dias`: o ritmo por degrau com o **passo** como contraprova, a grade como rótulo por cima do contrato — mesma contagem e mesma soma de `v_demanda_projetada` —, o material INATIVO que continua na grade de propósito, o `mes` idêntico ao `group by` da rotina, a **paridade total ↔ detalhe célula a célula e por regra**, e as duas reduções silenciosas, que são diferentes) e `projecao_test` + `tela_projecao_test` (o pivô material × mês, o filtro por regra agindo ANTES do pivô, os três vazios, o drill-down por célula e o teste de 390 px) | **8.5** ✅ — o §17 não previa arquivo para o 8.5; é a quinta vez da mesma família (`053`, `061`, `062`, `072`): tela planejada sem objeto de banco que acaba com views próprias, e `views-leitura.md` §12.1 diz que view de tela é do card da tela. Mora no bloco `08x`, ao lado do `080` e do `081`, não no `095`. ⚠️ **Quatro contraprovas vistas vermelhas:** `coalesce(ritmo_dias, 30)` no detalhe (o `—` de PREVISAO_CURSO/MODULAR vira número), o `mes` deslocado em um mês (reprova a definição **e** a paridade), `where m.ativo` na grade (reprova a exceção declarada) e o `encerrar_sessao` removido da §4. ⚠️ **Duas contraprovas passaram VERDE primeiro, e as duas ensinaram coisa diferente.** A da §4: as sete asserções de RLS rodavam depois de `tests.como_rotina`, e **dentro do contexto de rotina `tem_permissao()` é sempre verdadeira** — `semperfil@` via as 7 linhas da grade e as 49 do detalhe, e a paridade "todos veem o mesmo" passava porque todos viam tudo. É o mesmo achado do 5.4 e do 8.3 por uma terceira via, e a correção é uma linha (`tests.encerrar_sessao()` antes da seção). A do `mes`: a primeira sabotagem era `data_prevista + 15`, e **toda** data prevista da fixture cai antes do dia 16 — somar 15 dias não muda o bucket de nenhuma linha, a view sabotada era idêntica à correta sobre estes dados, e teste nenhum detecta mudança que não muda nada. Contraprova de agrupamento por mês desloca um **mês**, não alguns dias | 8 |
| `073_aluno_turmas` (a view das duas formas de turma do lado do aluno: Eduarda Lima fora de bloco e DENTRO da turma Modular, a **paridade** "sem turma pela view = `ALUNO_SEM_TURMA` aberta pela rotina" em três estados — fixture, turma Modular desativada, aluna fora da turma —, com guarda de conjunto não vazio, e a RLS na pele: direção, monitor, outra unidade, perfil sem `turmas.ler` e perfil com `turmas.ler` sem `alunos.ler`, que não vê **nenhuma** das duas formas) e `turmas_test` + `tela_alunos_test` (o aluno Modular sem ⚠ em 1400 e 390 px, a aba com a seção "Turma Modular" e sem o aviso de pendência) | **9.2,6** ✅. ⚠️ **Três contraprovas vistas vermelhas no banco:** a view sem a metade Modular (a paridade e Eduarda reprovam), `turma_ativa` constante (a paridade com a turma desativada reprova) e a metade Modular sem o `join` em `aluno` (o perfil só-turmas passa a ver a Modular sozinha). **Uma no app:** `alunosEmTurma` ignorando Modular derruba os testes da lista e da aba. ⚠️ **Achado de passagem:** `tela_bloco_test` fixava a semana de 07/09/2026 e o aviso de data retroativa passou a aparecer sozinho depois de 09/09 — a semana do painel virou a **próxima**, relativa a `hojeSaoPaulo()` | 9 |
| `083_certificado_fila` (a view da tela 9: o panorama da fila numa linha só, o aluno **sem trilha** que não é fim de curso, o FORMADO e os status parados que saem, as cinco colunas do checklist **nulas** e depois `false`/`NAO_PEDIDO`, a data derivada da última entrega, a paridade view × tabela, a segunda situação produzida entregando o último livro na própria transação, e as **três** reduções silenciosas) e `certificados_test` + `tela_certificados_test` (os rótulos das duas situações, o filtro "financeiro pendente" que **inclui quem não tem checklist**, a ordem com nulo por último, a permissão POR ITEM na pele do monitor, e o teste de 390 px com a caixa Financeiro acionável na lista) | **8.6** ✅ — o §17 não previa arquivo para o 8.6; é a **sexta** vez da mesma família (`053`, `061`, `062`, `072`, `082`). Mora no bloco `08x`, ao lado do `081`, que é o arquivo da tabela. ⚠️ **Quatro contraprovas vistas vermelhas:** sem o `exists` da trilha (dois ATIVOS sem trilha entram como formandos), `join` no lugar de `left join` (metade da fila some e a redução silenciosa de `certificados.ler` deixa de ser mensurável), `FORMADO` no filtro de status (todo formando da história fica na fila) e `pend.qtd < 1` no lugar de `<= 1` (a metade ULTIMO_LIVRO some, e com ela o "dá tempo de pedir"). ⚠️ **Uma armadilha de forma, herdada do `081`:** o `t_ids` precisa ser **tabela** temporária, e não view — a seção 4 chama `fn_certificado_abrir` já em `authenticated`, e nesse papel o schema `tests` não é alcançável; como view, a chamada morre em «permission denied for function unidade» | 8 |
| `084_dashboard_views` (as três views do Dashboard completo: o panorama de cada uma numa linha só, `em_ultimo_livro` ≠ `em_fim` com números **opostos** por método, as duas colunas refeitas contra a trilha real, a invariante *ativos + acelerar = `sem_previsao` + soma dos semestres* **por método**, a vencida que fica no semestre dela, o ano/semestre lidos da própria data do aluno, o TRANCADO com previsão que não entra, as **alocações** que contam duas vezes em aceleração e a desativada que sai) e `dashboard_test` + `tela_dashboard_test` (a junção por `metodo_id`, a ordem por alunos em curso, a agregação cronológica dos semestres, o atalho do "standby" para Alunos filtrado, os dois estados vazios, a falha que não vira zero, os rótulos de leitura de tela e o teste de 390 px) | **8.7** ✅ — o §17 não previa arquivo para o 8.7; é a **sétima** vez da mesma família (`053`, `061`, `062`, `072`, `082`, `083`), com uma diferença: aqui as views estavam previstas em `views-leitura.md` §8 desde 01/09/2026, com o card ao lado — faltava o arquivo de teste. Mora no bloco `08x`, entre o `083` e o `085`. ⚠️ **Cinco contraprovas vistas vermelhas:** `em_ultimo_livro`/`em_fim` trocadas (reprova três asserções da §2), a previsão vencida **descartada** do `qtd_alunos` (reprova cinco, incluindo a conta que fecha), `count(distinct aluno_id)` no lugar de `count(*)` nos tipos, o `where ba.ativo` removido, e o `join` em `metodo` virando `left join` (esta no `095`). ⚠️ **A terceira é a que mais ensina:** a fixture do card 5.1 pôs alunos **disjuntos** nos dois blocos cheios, então na partida "19 alocações" e "19 alunos" são o mesmo número e a sabotagem passaria verde — o arquivo **cria a aceleração** (segundo bloco de Ana Paula, por `fn_bloco_admitir`) para separar os dois, e só então ela fica vermelha | 8 |
...
| `092_rotina_sob_demanda` (a direção executa a rotina da SUA unidade na hora com `fn_rotina_diaria_executar` — projeção sai do zero, a da outra unidade não se mexe —, secretaria e monitor recebem `SEM_PERMISSAO` na pele, `rt_diaria(p_unidade)` roda só a unidade pedida e continua existindo UMA vez para o cron), `tests_concorrencia/rotina_diaria_dupla.sh` (duas sessões da direção: a segunda recebe `JA_EM_EXECUCAO` sem esperar; uma terceira, depois, roda — a trava é de transação) e `recalcular_agora_test` + presença em `tela_projecao_test` e `tela_importacao_test` (o botão só com `parametros.gerir`, EXECUTADA recarrega seis versões e JA_EM_EXECUCAO nenhuma, desabilitado com motivo enquanto roda, 390 px e desktop) | **9.2,65** ✅. ⚠️ **Três contraprovas vistas vermelhas no banco:** `rt_diaria` sem o filtro de unidade (a Escola B passa a ser calculada), a função sem `fn_exige_permissao` (secretaria e monitor passam) e as duas sem o `pg_try_advisory_xact_lock` (a segunda sessão devolve `EXECUTADA` depois de esperar as linhas da primeira). **Duas no app:** o botão sem a guarda de permissão e a recarga desligada. A trava entre duas sessões **não cabe no pgTAP**: o advisory lock é reentrante na mesma sessão | 9 |
| `093_vigiar_rotina` (o anon executa `fn_rotina_diaria_ultima_execucao` e o authenticated não; a assinatura sem parâmetro, uma linha, um `timestamptz`; fixture → NULL; a execução sob demanda NÃO carimba e a completa carimba cada unidade ativa; a data devolvida é a da unidade mais atrasada; a unidade com `rt_*` sabotada não carimba; unidade ativa sem carimbo → NULL; anon não lê a tabela; direção vê só a linha da sua unidade e secretaria nenhuma) e `worker-vigia/test` (a sonda da rotina: asserção positiva, 36 h passa e 37 reprova, `null`/404/futuro/corpo torto reprovam, só nos ambientes de pé, e-mail próprio com o SQL de conferir) | **9.2,66** ✅. ⚠️ **Três contraprovas vistas vermelhas no banco:** carimbar na execução sob demanda, carimbar com `rt_*` falhando, e `max` no lugar de `min` sem o NULL. **Duas no vigia:** a rotina fora do `algoRuim` e o `null` aceito | 9 |
| `tests_ponta_a_ponta/extrator_importador.mjs` (o `.xlsx` sintético do extrator → CLI de verdade → JSON no disco → `fn_importacao_registrar`/`aplicar` na MATRIZ, com rollback. A: a fixture como está sai REPROVADA e todo ERRO do importador já estava no relatório do extrator; B: com a entrada de abertura e os PCs pré-carregados, VALIDADA, APLICADA, totais por entidade e reexecução sem duplicar) e `transformacao.test.mjs` (`SALDO_NEGATIVO` e `SALA_SEM_PC`) | **9.2,67** ✅ — passo próprio no job `banco`. ⚠️ **A primeira execução achou dois defeitos da própria fixture:** saldo negativo (V10 reprovava) e sala sem PC (a APLICAÇÃO caía em `BLOCO_LOTADO`, com a validação verde). **Contraprovas vistas vermelhas:** um campo do aluno renomeado no extrator (`metodo` → `metodo_aluno`) e o extrator sem `conferirSaldo` (A acusa a surpresa) | 9 |
| `101_importacao_guardas` (a validação chamada de novo é recusada com `IMPORTACAO_JA_VALIDADA` e as ocorrências NÃO dobram, também em lote APLICADO; secretaria ouve `SEM_PERMISSAO`; o `for update` no corpo de `fn_importacao_aplicar`; `v_importacao_ocorrencia` com paridade de linhas com a tabela e RLS na pele — secretaria, monitor e direção da outra unidade veem zero) e `tests_concorrencia/importacao_aplicar_dupla.sh` | **9.2,68** ✅. ⚠️ **Contraprovas vistas vermelhas:** as duas funções na definição anterior (5 asserções reprovam, inclusive a duplicação das ocorrências, medida) e a aplicação sem `for update` na suíte de concorrência — a segunda sessão aplicava o lote de novo e devolvia APLICADA | 9 |
| `102_importacao_retencao` (`rt_importacao_retencao`, sexto passo de `rt_diaria`, pela porta de verdade — `fn_rotina_diaria_executar()` na pele da direção: limpa só APLICADA com 30 dias ou mais (30 limpa, 29 fica), guarda totais, ocorrências e carimbos, não mexe em VALIDADA/FALHOU — nem FALHOU com `aplicado_em` —, nem na outra unidade, não carimba duas vezes; sem o parâmetro, nada se apaga e abre `ROTINA_FALHOU`) | **11.5,5** ✅ — ⚠️ quatro contraprovas vistas vermelhas; a do filtro de status passou **verde** primeiro e ganhou o lote `A_FALC` | 11 |
| `094_ritmo_metodo` (`fn_ritmo_metodo_observado`: mediana 60 de 5 intervalos na fixture, onde a média seria 70; Inglês e Modular "sem base" — nulo e 0; janela padrão de `ritmo_calibracao_dias` por paridade com a conta à mão; `ritmo_intervalo_max_dias` = 90 tira o intervalo de 100; sem o parâmetro, `PARAMETRO_AUSENTE`; direção B com o método da A não vê nada da A; secretaria e monitor `SEM_PERMISSAO`) | **9.2,69** ✅ (parte (a) do 9.5). ⚠️ **Três contraprovas vistas vermelhas:** `avg` no lugar da mediana (a cauda da fixture separa 60 de 70), sem o filtro de unidade e sem `fn_exige_permissao` | 9 |
| `096_views_colunas_tela` (`dias_aberta` = 1 para pendência aberta ontem às 22h de São Paulo — 01h de hoje em UTC; `metodo_codigo`/`metodo_nome` da grade = o método do material; `data_inicio` da lotação = a coluna da turma; `modular_sem_cronograma` falso para o aluno no cronograma e verdadeiro quando ele sai da turma) + `tela_projecao_test` (a coluna Método com o catálogo em erro; o aviso na linha do drill-down, 390 px e desktop) e `tela_turmas_modular_test` (a edição mostra o início real e grava a correção, 390 px e desktop) | **9.2,7** ✅. ⚠️ **Contraprovas vistas vermelhas:** o cast antigo `criado_em::date` (dias_aberta = 0) e a marca constante no banco; no app, sem o aviso no drill-down e o campo de início só na criação. **Achado de passagem:** o formulário da turma Modular estourava 69 px em 390 px — os dois dropdowns sem `isExpanded` mediam o item mais longo | 9 |
| `diretrizes_acessibilidade_test` (contraste AA, rótulo em todo alvo e alvo mínimo — 44 px no celular, 40 no desktop — no Dashboard e na lista de Alunos, 390 px e desktop, com o tema da faixa) + `texto_de_tela_test` (varredura estendida a `lib/` inteiro, "Fase N", e a interpolação com aspas dentro) + badge, gaveta "Mais", célula "tentar de novo" e ritmo carregando | **9.2,72** ✅. ⚠️ **A primeira execução das diretrizes achou o contraste 2,96 dos zeros do 9.2,71**; um segundo "achado" (o "Filtrar" com 40 px) era artefato do tema compacto no teste e sumiu com o tema da faixa. **Contraprovas vermelhas:** badge sem `excludeSemantics` e gaveta sem `selected` | 9 |
| `012_catalogo_contratos` (C12, o lado SQL: os códigos que as funções levantam no DETAIL = a lista entre as marcas `<catalogo>`, sem sobra nem falta) + `catalogo_erros_test` (a lista do 012 = o fixture) + `telas_sessao_test` (login, redefinir senha, acesso bloqueado nos três modos, seleção de unidade e "Sem acesso", 390 px e desktop) + 390 px em Administração, Pendências, Bloco e Turmas + estado de erro em Administração (e o vazio "Só você por aqui"), Pendências, Salas e Materiais (Cursos e Combos) | **9.2,73** ✅. ⚠️ **Contraprovas vermelhas nos dois elos do C12:** um código tirado da lista do 012 (o pgTAP acusa `SEM_PERMISSAO` levantado e fora do catálogo) e um código trocado na lista (o teste Dart acusa a divergência com o fixture). Medido na partida: 65 códigos no banco e 65 no fixture, sem divergência — o que faltava era o portão | 3 |
| `097_nomes_usuarios` (`fn_usuarios_nomes` devolve só `TABLE(id uuid, nome text)`; secretaria e monitor recebem todos os nomes da Escola A e nenhum da B; `usuario` continua fechada para eles; direção B, exatamente a B; anon não executa; `v_material_movimento.criado_por_nome` chega para a secretaria) + `nomes_usuarios_test` | **9.2,74** ✅. ⚠️ **Contraprovas vermelhas:** sem o filtro de unidade e a view com o join antigo em `usuario`. **Achado no próprio teste:** a asserção da direção B com `join usuario` passou VERDE na sabotagem — o join filtrava pela RLS de quem lê e escondia o vazamento; reescrita pela contagem exata. Medido também pelo REST na pele da secretaria (embed antigo: `null`; função: os 7 nomes) | 9 |
| `098_projecao_acuracia` (`v_projecao_acuracia`: o retrato previsto × realizado da Escola A com fotos escritas à mão; o entregue **sem** previsão aparece com erro negativo (full join) e o previsto não entregue com erro positivo; material previsto por duas regras é UMA linha e o Σ realizado do mês é o das entregas; só a foto de UM mês antes conta (nem a do próprio mês, nem a de dois antes); mês sem foto anterior não é medido; mês corrente fica fora; RLS na pele da direção das duas escolas) | **11.1,5** ✅ — ⚠️ três contraprovas vistas vermelhas: `left join`, `group by … regra` e sem o filtro de mês medido | 11 |
...
| ~~Reexecutabilidade da importação~~ | **9.1** ✅ (06/09/2026) — virou o arquivo `100_importacao` (33 asserções), que abre o bloco `10x`. A reexecutabilidade é medida de duas formas: o mesmo snapshot importado duas vezes devolve `no_sistema` **idêntico**, e o movimento de estoque (a única entidade sem chave natural, e imutável) **não duplica**. O 9.4 continua sendo quem a mede sobre o recorte real | 9 |
| `100_importacao` (o conjunto exato da rota medido por paridade — só a direção passa, e a secretaria tem os quatorze de escrita —; o arquivo limpo com os três avisos; a **simulação que escreve e desfaz**, com as contagens antes e depois; a aplicação com os totais; a segunda importação do mesmo snapshot; o arquivo torto com uma família de erro de cada tipo no mesmo relatório; e o trigger recusando uma linha, que desfaz a transação inteira e vira ocorrência) e `importacao_test` + `tela_importacao_test` (a leitura do arquivo, o CSV do relatório, os quatro passos, o botão de aplicar que exige simulação e o teste de 390 px) | **9.1** ✅ — card de **Tela** com objeto de banco próprio, a oitava vez da mesma família (`053`, `061`, `062`, `072`, `082`, `083`, `084`). ⚠️ **Quatro contraprovas vistas vermelhas:** `admin.ler` trocado por `alunos.ler` no conjunto (a secretaria passa a enxergar lote), a simulação sem o `raise` que a desfaz (as duas contagens antes/depois quebram), a dedup de `movimento_estoque` removida — e aí o **achado**: sem ela a segunda importação **não duplica, ela falha**, porque a unique de `importacao_referencia` recusa a chave repetida — e a V10 sem o desconto do que já foi importado. ⚠️ **Esta última PASSOU VERDE na primeira tentativa**, e é o achado mais útil do card: com material **novo**, o saldo depois da primeira carga é exatamente a soma do arquivo, e `S + S` nunca é negativo quando `S` não é. O caso só se separa quando o material **já tem saldo e o arquivo só o consome** — o teste ganhou um terceiro arquivo para isso. ⚠️ **E uma armadilha de forma:** função **volátil** dentro de `where` é executada uma vez POR LINHA — `where i.id = fn_importacao_registrar(...)` criava um lote por linha de `importacao` e comparava com um id diferente a cada vez, devolvendo NULL. O NULL fazia a asserção falhar **com a sabotagem e sem ela**, e a contraprova teria "passado" pela razão errada | 9 |
...
2. **Acessibilidade automatizada** — o card 2.7 verificou contraste par a par no papel; um teste
   ...
   ✅ **Trazido para dentro no card 9.2,72 (24/09/2026):** `app/test/diretrizes_acessibilidade_test.dart`
   roda `textContrastGuideline`, `labeledTapTargetGuideline` e o alvo mínimo (44 px no celular, 40 px
   no desktop) sobre o Dashboard e a lista de Alunos, em 390 px e desktop, com o tema da faixa. A
   primeira execução achou um defeito real recém-introduzido: as linhas zeradas do Dashboard (9.2,71)
   com transparência 0,75 tinham contraste 2,96. Tela nova entra com uma linha na lista do teste.
