...
| 4 ✅ | `fn_param_int`/`fn_param_txt` eram `security invoker` e leem `parametro`: `fn_rep_situacao` (invoker, chamada pela tela) devolveria `PARAMETRO_AUSENTE` para quem não tem `parametros.ler`. **Corrigido no card 3.4** — as duas nasceram `security definer` com `search_path` fixo — e **medido só em 06/09/2026 (marco 8.8)**, que é o atraso que esta linha registra: a correção estava no código e a tabela seguia dizendo "alta", e o critério de REPROVA do M3 (`estrategia-testes.md` §15.3) ainda citava o invoker como bloqueante conhecido. `parametros.ler` é **só da direção**, e ainda assim os quatro perfis abrem a tela 8 com o mesmo número e leem `fn_param_int('projecao_horizonte_dias')`. ⚠️ **Quem de fato dependia disso é a tela 7**, não a 8: a janela da parcela projetada de `v_pedido_sugerido` (card 8.2) chama `fn_param_int` **sem default**, e como `invoker` a secretaria — um dos dois perfis da rota — receberia `PARAMETRO_AUSENTE` ao abrir Compras. **Asserido** em `095` §9, com o guarda que prova que o perfil da contraprova não tem a permissão (senão o par mediria nada) e com a sabotagem para `invoker` vista **vermelha** | 2.2 §2.3 | 3.4 / 8.8 | alta |
...
| 7 ✅ | `pendencia` é aberta por quase toda função de aplicação: `insert` sem exigência de domínio — **feito no card 5.5**: `pendencia_ins` exige só a unidade (`20260903234500_pendencias_rotinas.sql`) | 2.1 §4 | 5.5 | **bloqueante** |
