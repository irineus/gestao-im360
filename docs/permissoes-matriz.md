...
| 6 | `bloco_aluno`, `turma_modular_aluno` e `bloco_aluno_reposicao` são desativados por `tg_aluno_status_desaloca`: `update` precisa aceitar também `alunos.alterar_status`/`alunos.reverter_status` | 2.1 §4 | 5.1 / 7.1 | **bloqueante** |
| 7 | `pendencia` é aberta por quase toda função de aplicação: `insert` sem exigência de domínio | 2.1 §4 | 5.5 | **bloqueante** |
| 8 | RLS não é por coluna: com `certificados.marcar_financeiro` o monitor pode `PATCH` `pedagogico_ok` direto no PostgREST. Exige `tg_certificado_colunas_permitidas` (`before update`), comparando OLD/NEW por grupo de colunas e chamando `fn_exige_permissao` | 2.2 §9 | 8.3 | alta |
| 9 | `movimento_estoque` precisa de política de `insert` **por tipo**, não de um `estoque.criar` genérico | 2.1 §4 | 6.1 | alta |
