-- =============================================================================
-- Importação: guarda da validação chamada direto e trava da aplicação
-- (card 9.2,68)
--
-- Dois achados da varredura de 24/09/2026 sobre 20260906120000_importacao.sql,
-- nenhum visto em uso porque a tela chama as funções na ordem certa — e os dois
-- alcançáveis por quem tem o conjunto da rota 13, pelo PostgREST:
--
--   (1) fn_importacao_validar tem grant para authenticated e não conferia o
--       estado do lote: chamada de novo, duplicava as ocorrências, inclusive em
--       lote APLICADO. Agora exige o conjunto da rota (fn_importacao_exigir) e
--       recusa lote já validado com IMPORTACAO_JA_VALIDADA;
--   (2) fn_importacao_aplicar não travava a linha do lote: duas aplicações
--       simultâneas liam VALIDADA e a segunda aplicava o lote de novo,
--       devolvendo APLICADA (medido) — ou, com movimento de estoque, parava
--       na unique de importacao_referencia com erro de chave. Agora
--       `for update`, e a segunda recebe IMPORTACAO_JA_APLICADA.
--
-- As duas funções são substituídas INTEIRAS a partir da única definição
-- anterior (20260906120000_importacao.sql); o resto do corpo não mudou. O
